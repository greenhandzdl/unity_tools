...
public class CreateFolders : EditorWindow // 声明一个名为CreateFolders的类，继承自EditorWindow，用于创建自定义编辑器窗口喵
{
    ...
    [MenuItem("Assets/ACreate/Folder/Create Default Folders", priority = 1)] // 添加一个菜单项到Assets/ACreate/Folder菜单下，点击后会调用SetUpFolders方法喵，并设置优先级
    ...
    private static void SetUpFolders() // 声明一个静态方法SetUpFolders，用于创建编辑器窗口喵
    {
        ...
        {
            Menu.SetChecked("Assets/ACreate/Folder/Create Default Folders",false);
            Menu.SetChecked("Tools/Create/Folder/Create Default Folders",false);
